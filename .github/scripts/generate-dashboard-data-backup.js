...
const env = {
  GITHUB_REPOSITORY:
    process.env.GITHUB_REPOSITORY || "trinity-method/trinity-dashboard",
...
try {
  ...
  console.log(
    `- Health Score: ${data.health.score}/100 (${data.health.level})`,
  );
  console.log("");
  ...
  const minimal = {
    ...
    tests:
      data.tests.total > 0 ? `${data.tests.passed}/${data.tests.total}` : "N/A",
