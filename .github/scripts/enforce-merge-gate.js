...
function generateIssuesList(report) {
  ...
  if (report.breakdown) {
    if (report.breakdown.coverage < 80) {
      issues.push(
        `❌ Coverage is ${
          report.prData?.coverage?.toFixed(1) || 0
        }% (minimum 80%)`,
      );
    }
    if (report.breakdown.tests < 100) {
      issues.push(
        `❌ Test pass rate is below 100% (${report.prData?.tests || "N/A"})`,
      );
    }
    if (report.breakdown.security < 80) {
      issues.push(
        `❌ Security score is ${report.prData?.security || 0}/100 (minimum 80)`,
      );
    }
...
| Coverage | ${report.breakdown.coverage.toFixed(1)}% | ${
    report.weights.coverage
  } | ${(report.breakdown.coverage * report.weights.coverage).toFixed(1)}% |
| Tests | ${report.breakdown.tests.toFixed(1)}% | ${report.weights.tests} | ${(
    report.breakdown.tests * report.weights.tests
  ).toFixed(1)}% |
| Security | ${report.breakdown.security.toFixed(1)}% | ${
    report.weights.security
  } | ${(report.breakdown.security * report.weights.security).toFixed(1)}% |
| Code Quality | ${report.breakdown.codeQuality.toFixed(1)}% | ${
    report.weights.codeQuality
  } | ${(report.breakdown.codeQuality * report.weights.codeQuality).toFixed(
    1,
  )}% |
...
${
  score >= 90
    ? "🎉 **Excellent** - This PR exceeds quality standards!"
    : score >= 80
      ? "✅ **Good** - This PR meets quality standards and is ready to merge."
      : score >= 70
        ? "⚠️ **Acceptable** - This PR barely meets minimum standards. Consider improvements."
        : score >= 60
          ? "🔧 **Needs Work** - This PR requires significant improvements before merging."
          : "❌ **Not Ready** - This PR does not meet minimum quality standards."
}

---
*Generated by Trinity Dashboard Merge Gate | [View Dashboard](https://your-dashboard-url.com/pr/${
  ...
  try {
    ...
    console.log(
      `GitHub comment ${botComment ? "updated" : "created"} successfully`,
    );
  } catch (error) {
  ...
  try {
    await octokit.rest.checks.create({
      ...
      output: {
        ...
        summary: `This PR ${
          isPassing ? "meets" : "does not meet"
        } the merge readiness threshold of ${MERGE_THRESHOLD}%.
...
async function enforceMergeGate() {
  try {
    ...
    if (!meetsThreshold) {
      ...
      console.log(
        `This PR cannot be merged because the readiness score is ${readinessScore}%`,
      );
      ...
      console.log(
        `✅ Readiness score ${readinessScore}% meets the threshold of ${MERGE_THRESHOLD}%`,
      );
      core.info(
        `✅ Merge allowed: Readiness score ${readinessScore}% meets threshold`,
      );
    }
