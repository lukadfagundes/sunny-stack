...
function calculateCodeQualityScore(prData) {
  ...
  if (prData.linting) {
    const lintScore =
      ...
      Math.min(
        50,
        (prData.linting.errors || 0) * 10 + (prData.linting.warnings || 0) * 2,
      );
...
function main() {
  try {
    ...
    console.error(
      `- Coverage: ${result.breakdown.coverage.toFixed(1)}% (weight: ${
        WEIGHTS.coverage
      })`,
    );
    console.error(
      `- Tests: ${result.breakdown.tests.toFixed(1)}% (weight: ${
        WEIGHTS.tests
      })`,
    );
    console.error(
      `- Security: ${result.breakdown.security.toFixed(1)}% (weight: ${
        WEIGHTS.security
      })`,
    );
    console.error(
      `- Code Quality: ${result.breakdown.codeQuality.toFixed(1)}% (weight: ${
        WEIGHTS.codeQuality
      })`,
    );
