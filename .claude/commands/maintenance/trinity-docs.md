...
const file_discovery_result = retry_operation(
  ...
  () => {
    ...
    const total =
      files.diagrams.length + files.guides.length + files.api.length;

    if (total === 0) {
      throw new Error(
        "No files discovered - may be transient filesystem delay",
      );
    }
...
for (const file of diagram_files) {
  ...
  const has_valid_type = valid_types.some((type) =>
    mermaid_block.startsWith(type),
  );
  ...
  if (
    mermaid_block.includes("{{") ||
    mermaid_block.includes("[Add ") ||
    mermaid_block.includes("[Define ")
  ) {
    verification_issues.mermaid_errors.push({
      file: file,
...
for (const file of all_docs) {
  ...
  for (const pattern of stub_patterns) {
    ...
    if (matches) {
      ...
      const has_future_section = content.match(
        /##+ Future Enhancements|##+ Roadmap|##+ Coming Soon/i,
      );
      ...
      if (has_future_section) {
        ...
        const future_section_index = content.search(
          /##+ Future Enhancements|##+ Roadmap|##+ Coming Soon/i,
        );
      ...
      if (!in_future_section) {
        verification_issues.stub_content.push({
          ...
          context: content.substring(
            Math.max(0, content.search(pattern) - 50),
            Math.min(content.length, content.search(pattern) + 100),
          ),
        });
      }
    }
...
const endpoint_diff = Math.abs(
  documented_endpoint_count - actual_endpoint_count,
);
...
const component_diagrams = diagram_files.filter((f) =>
  f.includes("component-hierarchy"),
);
...
const component_diff = Math.abs(
  documented_component_count - actual_component_count,
);
...
if (paragraph_lengths.length > 0) {
  const avg_length =
    paragraph_lengths.reduce((a, b) => a + b, 0) / paragraph_lengths.length;
...
LOG: "";
LOG: "Quality Score: " +
  global.trinity_docs_session.verification.score +
  "/100";
