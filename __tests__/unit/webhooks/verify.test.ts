...
describe("Webhook Signature Verification", () => {
  ...
  describe("verifyGitHubWebhook", () => {
    ...
    test("should be timing-safe against timing attacks", () => {
      ...
      expect(Math.abs(timeDiff)).toBeLessThan(5.0);
    });
  });
  ...
  describe("Edge Cases", () => {
    test("should handle null or undefined gracefully", () => {
      ...
      expect(() =>
        verifyGitHubWebhook(null as any, "sig", secret),
      ).not.toThrow();
      expect(() =>
        verifyGitHubWebhook("payload", null as any, secret),
      ).not.toThrow();
      expect(() =>
        verifyGitHubWebhook("payload", "sig", null as any),
      ).not.toThrow();
