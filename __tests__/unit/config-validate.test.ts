 ...
 *
 * Tests for lib/config-validate.ts - validates all 37 environment variables
...
describe("config-validate", () => {
  ...
  describe("Google OAuth Variables (4)", () => {
    ...
    test("should validate GOOGLE_PROJECT_ID as required", () => {
      // ARRANGE
      Object.assign(process.env, createValidEnv());
      ...
      // ASSERT
      expect(result.valid).toBe(false);
      expect(hasError(result.errors, "GOOGLE_PROJECT_ID")).toBe(true);
    });
  });
  ...
  describe("Discord Channel Variables (13)", () => {
    ...
    test.each(channelIds)(
      "should validate %s as required with min 15 characters",
      (channelId) => {
        // ARRANGE
        Object.assign(process.env, createValidEnv());
  ...
  describe("Deployment API Variables (2)", () => {
    test("should validate GITHUB_API_TOKEN as required with min 20 characters", () => {
      // ARRANGE
      Object.assign(process.env, createValidEnv());
      process.env.GITHUB_API_TOKEN = "short";
      ...
      // ASSERT
      expect(result.valid).toBe(false);
      expect(hasError(result.errors, "GITHUB_API_TOKEN")).toBe(true);
    });

    test("should validate VERCEL_API_TOKEN as required with min 20 characters", () => {
      // ARRANGE
      Object.assign(process.env, createValidEnv());
      process.env.VERCEL_API_TOKEN = "short";
      ...
      // ASSERT
      expect(result.valid).toBe(false);
      expect(hasError(result.errors, "VERCEL_API_TOKEN")).toBe(true);
    });
  });
  ...
  describe("Summary Statistics", () => {
    test("should report correct total of 37 variables", () => {
      // ARRANGE
      Object.assign(process.env, createValidEnv());
      ...
      // ASSERT
      expect(result.summary.total).toBe(37);
    });
    ...
    test("should count configured variables correctly", () => {
      ...
      // ASSERT
      expect(result.summary.configured).toBeGreaterThanOrEqual(32); // Required + some optional
      expect(result.summary.configured).toBeLessThanOrEqual(37);
    });
  ...
  describe("Error Messages", () => {
    test("should provide helpful error messages for URL validation", () => {
      ...
      expect(
        result.errors.some(
          (e) => e.includes("DATABASE_URL") && e.toLowerCase().includes("url"),
        ),
      ).toBe(true);
    });
    ...
    test("should provide helpful error messages for length validation", () => {
      ...
      expect(
        result.errors.some(
          (e) => e.includes("NEXTAUTH_SECRET") && e.includes("32"),
        ),
      ).toBe(true);
    });
    ...
    test("should provide helpful error messages for format validation", () => {
      ...
      expect(
        result.errors.some(
          (e) => e.includes("ADMIN_EMAIL") && e.toLowerCase().includes("email"),
        ),
      ).toBe(true);
    });
  });
  ...
  describe("Complete Validation", () => {
    test("should pass validation with all 37 variables set correctly", () => {
      // ARRANGE
      Object.assign(process.env, createValidEnv());
 ...
 * Returns an object with all 37 variables set to valid values
 */
function createValidEnv(): Record<string, string> {
  return {
    ...
    // Deployment APIs (2)
    GITHUB_API_TOKEN: "a".repeat(20), // 20+ characters
    VERCEL_API_TOKEN: "b".repeat(20),
