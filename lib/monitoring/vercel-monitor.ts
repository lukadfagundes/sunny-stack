...
async function notifyDeploymentFailure(
  ...
  if (!VERCEL_NOTIFICATION_CHANNEL) {
  ...
  try {
    ...
    const channel = guild.channels.cache.get(VERCEL_NOTIFICATION_CHANNEL);
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        {
          name: '🎯 Target',
          value: deployment.target === 'production' ? '🟢 Production' : '🟡 Preview',
          inline: true
        }
      );
    ...
    logger.error('Failed to send Vercel deployment failure notification:', error);
  }
}
...
async function notifyProductionDeployment(
  ...
  if (!VERCEL_NOTIFICATION_CHANNEL) return;
  ...
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: '🌐 URL', value: `https://${deployment.url}`, inline: false }
      );
    ...
    logger.info(`Sent production deployment notification for ${deployment.name}`);
  } catch (error) {
    ...
    for (const deployment of recentDeployments) {
      ...
      if (
        ...
        const recoveryEmbed = new EmbedBuilder()
          ...
          .addFields({ name: '🌐 URL', value: `https://${monitored.url}`, inline: false })
          .setTimestamp(monitored.created)
          .setFooter({ text: 'Vercel Monitoring' });

        const channel = await client.channels.fetch(VERCEL_NOTIFICATION_CHANNEL!);
