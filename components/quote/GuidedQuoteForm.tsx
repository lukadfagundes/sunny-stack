...
const ProjectSection = lazy(() =>
  import("./sections").then((mod) => ({ default: mod.ProjectSection })),
);
const TimelineSection = lazy(() =>
  import("./sections").then((mod) => ({ default: mod.TimelineSection })),
);
const BudgetSection = lazy(() =>
  import("./sections").then((mod) => ({ default: mod.BudgetSection })),
);
const RequirementsSection = lazy(() =>
  import("./sections").then((mod) => ({ default: mod.RequirementsSection })),
);
...
export default function GuidedQuoteForm({
  onBack,
  onComplete,
}: GuidedQuoteFormProps) {
  debug.entry("GuidedQuoteForm", {
    onBack: !!onBack,
  ...
  const { currentStep, nextStep, previousStep, isFirstStep, isLastStep } =
    useMultiStepForm(guidedSteps.length);

  const validateCurrentStep = (): boolean => {
    debug.info("validateCurrentStep", "Validating current step", {
      step: guidedSteps[currentStep],
    });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(
          "Thank you! Your project request has been sent. I'll get back to you within 24 hours.",
        );
        onComplete();
      } else {
      ...
      alert(
        "There was an error sending your request. Please email luka@sunny-stack.com directly.",
      );
    } finally {
      setIsSubmitting(false);
    }
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-sunny-brown/80">
                Here's what I've gathered about your project
              </p>
            </div>
            ...
            <div className="bg-sunny-gold/10 rounded-lg p-6 space-y-3">
              <div>
                ...
                <span className="ml-2 text-sunny-brown/80">
                  {formData.name} ({formData.email})
                </span>
              </div>
              <div>
                <span className="font-medium text-sunny-brown">
                  Project Type:
                </span>
                <span className="ml-2 text-sunny-brown/80">
